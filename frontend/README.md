...
Open `backend/includes/db.php`, and make sure the following details match your MySQL setup:

```php
$host = '127.0.0.1';
$dbname = 'starmedia_jobfair_db';
$username = 'root';
$password = '';   // fill in if your MySQL has a password
```
...
| Larger version (modal/lightbox) | ✅ | `BaseModal.vue` — full floor plan via modal |
...
|---|---|---|
| Accessibility (keyboard nav, screen readers) | ⚠️ Partial | Semantic HTML + `aria-label`/`aria-expanded` on toggle buttons; no full keyboard trap for modals yet |
...
| Reduce quantity on a win | ✅ | `backend/api/spinning-wheel/spin.php` — updates `remaining` in the JSON |
